...
export class DatabaseStorage extends StorageInterface {
  ...
  async createCategory(category) {
    // 验证数据（创建时不需要ID）
    const validationResult = DataValidator.validate(category, {
      id: { required: false, type: 'string', maxLength: 100 },
      name: { required: true, type: 'string', maxLength: 100 },
      description: { required: false, type: 'string', maxLength: 500 }
    });

    if (!validationResult.isValid) {
      console.error('分类验证失败:', validationResult.errors);
      throw new StorageError(`分类数据验证失败: ${validationResult.errors.join(', ')}`, 'VALIDATION_ERROR');
    }
  ...
  async createQuestion(questionData) {
    try {
      // 验证数据（创建时不需要ID）
      const validationResult = DataValidator.validate(questionData, {
        id: { required: false, type: 'string', maxLength: 100 },
        categoryId: { required: true, type: 'string', maxLength: 100 },
        title: { required: true, type: 'string', maxLength: 200 },
        content: { required: true, type: 'string', maxLength: 2000 },
        difficulty: { required: true, type: 'string', maxLength: 20 },
        tags: { required: false, type: 'array' }
      });

      if (!validationResult.isValid) {
        console.error('问题验证失败:', validationResult.errors);
        throw new StorageError(`问题数据验证失败: ${validationResult.errors.join(', ')}`, 'VALIDATION_ERROR');
      }
